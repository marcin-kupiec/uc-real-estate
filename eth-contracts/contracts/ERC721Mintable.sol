...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "can be called only by contract owner");
        _;
    }

    constructor() internal {
        _owner = msg.sender;
        emit TransferredOwnership(address(0), _owner);
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        _owner = newOwner;
        emit TransferredOwnership(prevOwner, _owner);
    }
}
...
contract Pausable is Ownable {
    ...
    function setPause(bool flag) public onlyOwner {
        require(flag != _paused, "Pause status mode must be different from current one");
        _paused = flag;
        if (_paused) {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Address to approve cannot be an owner of the token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == getOwner() || isApprovedForAll(msg.sender, to), "Sender is not contract owner or isApprovedForAll() is false");
...
contract MarcinosERC721Token is ERC721Metadata {
